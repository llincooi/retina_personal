...
binshift_window = -60*0.1:1:60*0.1;
...
for z =26%1:n_file %choose file
    ...
    cc = 0*binshift_window;
    for bs = 1:length(binshift_window)
        [reconstrFrames, reconstrStimBins, populationFilter] = getLinearPopulationReadoutTimeShift(spikeCounts, stimFrames, filterLen, binshift_window(bs));
        %Correlation
        cc(bs) = mean((reconstrFrames-mean(reconstrFrames)).*(stimFrames(reconstrStimBins)-mean(stimFrames(reconstrStimBins)))/(std(stimFrames(reconstrStimBins))*std(reconstrFrames)));
        %         fullfig
        ...
        %         title(['Cross correlation is ',num2str(cc(bs))])
