...
displayset = 2;%:length(Fc_Sets)

for setnum = displayset
TheFileNames = ChooseWantedfile(Fc_Sets{setnum}.Including, Fc_Sets{setnum}.Excluding);
Files = cell(1, size(TheFileNames, 2)); 
Fc = [];
for i = 1: size(TheFileNames, 2)
    filename = convertStringsToChars(TheFileNames(i));
    iend = strfind(filename,'Hz');
    istart = strfind(filename(iend-6:iend),'_');
    Fc = [Fc str2num(filename(iend-6+istart:iend-1))];
    Files{i} = load(filename);
end

[Fc I] = sort(Fc);
if Fc(1) == 0 %%let OU (Fc==0) be the last one.
    I = [I(2:end) I(1)];
    Fc = [Fc(2:end) Fc(1)];
end
...
for channelnumber=1:60 %choose file
    ...
    hold off;
    saveas(gcf,['FIG\',Fc_Sets{setnum}.fig_name,'.tif'])
end
end
