...
HMM_MI =[];
HMM_MI_shuffle = [];
allchannellegend = cell(1,length(interruptratio));
...
MI_peaks = [];
peak_times = [];
interruptratio = sort(interruptratio);
for     C =1:length(interruptratio)
    if interruptratio(C) == 0
        load([HMM_former_name,num2str(G),HMM_middle_name ,'.mat'])
    else
        load([HMM_former_name,num2str(G),HMM_middle_name ,'_',num2str(interruptratio(C)) ,'interrupt.mat'])
    end
        ...
        allchannellegend{C} = ['C', num2str(interruptratio(C))];
...
MI_peaks=zeros(length(interruptratio),60);
ind_peak=zeros(length(interruptratio),60);
peak_times = zeros(length(interruptratio),60);
