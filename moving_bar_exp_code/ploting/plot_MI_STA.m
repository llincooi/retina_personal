...
%             newXpos = Monitor2DCoor2BarCoor(RGCs{i}.center_RF(1),RGCs{i}.center_RF(2),direction,'LCD');
            plot(STA_time,(mean(positive_PCAs{i},1)-newXpos)*micro_per_pixel,'r-.');hold on%Red is positive_PCA
            plot(STA_time,(mean(negative_PCAs{i},1)-newXpos)*micro_per_pixel,'b-.');%Blue is negative_PCA
        else
            plot(STA_time,(mean(positive_PCAs{i},1)-meaCenter_x)*micro_per_pixel,'r');hold on%Red is positive_PCA
            plot(STA_time,(mean(negative_PCAs{i},1)-meaCenter_x)*micro_per_pixel,'b');%Blue is negative_PCA
        end
        ...
        axes(ha(2)); 
        if sum(RGCs{i}.center_RF) >0
            plot(STA_time,(dis_STA(i,:)-newXpos)*micro_per_pixel,'k-.');
        else
            plot(STA_time,(dis_STA(i,:)-meaCenter_x)*micro_per_pixel);
        end
        xlabel('time before spike(sec)')
        ylabel('STA of bar')
        xline(0)
...
%         end
        ...
        if save_photo
           if i < 10
                 saveas(fig,[exp_folder,'\STA\unsort\MI\FIG\',name,'_ch0',int2str(i),'.tif'])
            else
                saveas(fig,[exp_folder,'\STA\unsort\MI\FIG\',name,'_ch',int2str(i),'.tif'])
           end
        end
