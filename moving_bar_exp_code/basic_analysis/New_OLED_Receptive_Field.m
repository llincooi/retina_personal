...
temporal_bias = sum(matcheckerboard, 1)/length(checkerboard); %No spatial bias because the algorithm makes sure same mean luminaous for each frame
matcheckerboard = matcheckerboard-temporal_bias;
gauss_RF = zeros(length(tstep_axis),side_length,side_length, 60);
GFS = 1.5; %sigma of Gaussian filter for smoothing
for k = displaychannel
    [n,~] = histcounts(analyze_spikes{k},[0, diode_BT]);
    for i =  tstep_axis  %for -50ms:-300ms
        sum_checkerboard = 0;
        for ii = 1:length(matcheckerboard)-2*i
            sum_checkerboard = sum_checkerboard + squeeze(matcheckerboard(ii,:,:))*n(ii+2*i);
        end
        gauss_RF(i,:,:,k) = imgaussfilt(sum_checkerboard/sum(n(1:length(matcheckerboard)-2*i)), GFS);
    end
end
%% calculate SVD
...
for k =displaychannel
    ...
    iTK = [0, squeeze(sum(gauss_RF(:,:,:,k), [2,3])).'];
    iTK = -iTK*iTK(2)/abs(iTK(2));
    plot([0 tstep_axis*fps*1000] , iTK/min(iTK)*min(SVD_TK(k, :)), 'LineWidth',1,'LineStyle','--');
...
%% plot RF(STK) & electrode position & RF center
for k = displaychannel
    ...
    for i = tstep_axis
        ...
        scatter(RF_properties(k).X_Coor, RF_properties(k).Y_Coor, 15, 'b' ,'o','filled')
    end
...
for k = displaychannel
    ...
    if strcmp(maindirection,'UL_DR')
        ...
        isk = diag(iSK,offset);
        isk = isk/min(isk);
        plot(isk,'LineWidth', 2)
        plot(diag(squeeze(SVD_SK(k, :, :)),offset)/max(diag(squeeze(SVD_SK(k, :, :)),offset)),'LineWidth', 2)
        ...
        bar(max(bc-hw,1):min(bc+hw, 1000)) = 1;
    ...
    elseif strcmp(maindirection, 'UR_DL')
        ...
        figure(300+k);hold on;
        isk = diag(fliplr(iSK),offset);
        isk = isk/min(isk);
        plot(isk,'LineWidth', 2)
        plot(diag(fliplr(squeeze(SVD_SK(k, :, :)))/max(diag(fliplr(squeeze(SVD_SK(k, :, :))),offset)),offset),'LineWidth', 2)
    ...
    elseif strcmp(maindirection, 'RL')
        ...
        isk = isk/min(isk);
        plot(isk,'LineWidth', 2)
        plot(squeeze(SVD_SK(k, offset, :))/max(squeeze(SVD_SK(k, offset, :))),'LineWidth', 2)
    ...
    if save_photo
        if sorted
            saveas(fig,[exp_folder, '\FIG\RF\', name,'\sort','\normalizedMBcut_ch', num2str(k)  '.tiff']) %#ok<UNRCH>
        else
            saveas(fig,[exp_folder, '\FIG\RF\', name,'\unsort','\normalizedMBcut_ch', num2str(k)  '.tiff']) %#ok<UNRCH>
        end
        close(fig);
    end
end
