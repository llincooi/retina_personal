...
for nn = 1:length(roi)
    ...
    if isempty(find(abs(cSTA(nn,1:round(length(cSTA)/2))) >= 3*std(cSTA(nn,1:300/bin))))
        cSTA(roi(nn),:) = NaN;
    else
...
if sorted
    saveas(gcf,['FIG\sort\', 'dcstadt',lumin,'.tiff'])
else
    saveas(gcf,['FIG\unsort\', 'dcstadt',lumin,'.tiff'])
end
...
% 
if sorted
    save([exp_folder,'\Analyzed_data\sort\','dcstadt',lumin,'.mat'],'time','cSTA', 'Filker_OnOff_Index', 'tau')
    saveas(gcf,['FIG\sort\', 'dcstadt',lumin,'on_off_index.tiff'])
else
    saveas(gcf,['FIG\unsort\', 'dcstadt',lumin,'on_off_index.tiff'])
    save([exp_folder,'\Analyzed_data\unsort\','dcstadt',lumin,'.mat'],'time','cSTA', 'Filker_OnOff_Index', 'tau')
end
